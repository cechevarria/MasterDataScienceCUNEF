...
#Objetivo:
1-Escoger el GDP y CPI de un país y realizar un modelo VAR
2- Holanda
...
#Series 
```{r}
series = c('NAEXKP01NLQ657S', 'NLDCPIALLQINMEI') # FRED codes for US GDP growth and CPI
...
pivot_longer(Data, cols=-Date, names_to="Variables", values_to="Values") %>% 
  ...
  labs(title="Paises Bajos: crecimiento del PIB - PCI inflación", x="", y="",
       caption=paste0("Fuente: FRED series ", paste(series, collapse=", ")))
```
...
ggplot(forecast_data) + 
  ...
  labs(title="Predicción de los Paises Bajos: crecimiento del PIB y PCI Inflación", 
       subtitle=paste("Datos trimestrales, porcentaje, VAR con", m, "retardos"), 
